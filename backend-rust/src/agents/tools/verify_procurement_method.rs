    ...
    fn definition(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "function": {
                ...
                "description": "【使用场景】校验标书声明的采购方式是否满足适用金额门槛——\
                    根据《政府采购法》第26/27/31条，对比项目预算金额与品类的公开招标数额标准，\
                    判断公开招标/竞争性谈判/竞争性磋商/询价/单一来源等采购方式是否合法适用。\
                    ① 货物200万/工程400万/服务100万为公开招标数额标准；\
                    ② 达到门槛必须公开招标（除非经财政部门批准）；\
                    ③ 低于门槛可采用竞争性谈判/磋商/询价；\
                    ④ 单一来源需满足5种法定情形之一。\
                    【不使用场景】不负责具体采购流程的执行步骤审阅；不验证采购方式的程序合规性（公告期限、文件发售期等——用 verify_announcement_period）。\
                    【注意】预算单位为万元（人民币），品类与方式必须使用中文枚举值。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "budget_amount": {
