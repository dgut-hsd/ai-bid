...
async fn run_review_pipeline(
    ...
    let coordinator = Arc::new(
        ...
        .with_chunk_map(review_chunk_map.clone()),
    );
    ...
    match coordinator.review(&review_clauses).await {
        Ok(mut output) => {
            ...
            for finding in &mut output.findings {
                ...
                for clause_id in &finding.clause_ids {
                    if let Some(chunk) = chunk_map.get(clause_id) {
                        ...
                        // 过滤占位 bbox（lopdf 失败降级），保留验证过的非占位 block
                        // 占位特征：x0==0.0 && x1==400.0 且高度 ≤20pt
                        for bid in &chunk.source_block_ids {
                            ...
                            if is_valid && !aggregated.contains(bid) {
                                aggregated.push(bid.clone());
                            }
                        }
                    }
                }
                // 全部 clause 都没有有效 block（如表格占位）→ 保持空，前端退化文本匹配。
                // 修复根因2：移除 max_blocks=5 截断，保留所有有效 block 以完整框选。
                finding.block_ids = aggregated;
            }
            ...
            // 顺序说明：usage 必须先算出来，下面写盘 result.json 时要带上它。
            let usage = {
            ...
            // 存入 review_results + review_usages 供 GET /result 查询（按租户隔离）
            {
            ...
            // 写盘: {doc_id}_result.json — 重启后磁盘 fallback（含 usage，落到租户目录）
            {
                let Some(dir) = tenant_output_path(&tenant_id, "findings") else {
                    return;
                };
